...
static atomic_t st_relay_deferred_late;
...
{
	atomic_inc(&st_relay_deferred_late);
}
...
void meshtastic_sched_stats_get(struct meshtastic_sched_stats *out)
{
	...
	out->relay_deferred_late = (uint32_t)atomic_get(&st_relay_deferred_late);
...
void meshtastic_sched_stats_reset(void)
{
	...
	atomic_set(&st_relay_deferred_late, 0);
	for (int i = 0; i < MT_RELAY_GAP_BUCKETS; i++) {
