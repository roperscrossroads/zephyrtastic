...
([`Kconfig.power`](../src/Kconfig.power)); the shipping profile
`overlay-v4-unified.conf` sets it `y` so a PM build sleeps out of the box, and the
phone's stored value overrides
