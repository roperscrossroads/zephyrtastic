...
| **Reset cause** | Logged to NETLOG at boot + `hwinfo reset_cause` on the shell on demand. Since 2026-08-30 the bootlog (`meshtastic resets`, retained-RAM ring + a FLASH ring) records every boot's cause and the previous run's length. |
| **Faults no handler sees** | `CONFIG_XTENSA_FAULT_BREADCRUMBS` (carried patch 0017, a default in every ESP32 board's `Kconfig.defconfig`, 2026-09-03): the double-exception vector records EXCCAUSE/EPC1/DEPC/EXCVADDR/PS/thread/nesting into RTC memory before it spins, and the interrupt C handlers keep per-level entry/dispatch counters with the last IRQ per level. Both are printed by `meshtastic resets` and the boot log. Exists because six hardware-watchdog resets landed with consoles attached and nothing printed — a double exception on Xtensa runs no handler, so FATAL, crashinfo and the coredump are structurally blind to it (tooling `RF-PARITY-PLAN.md` §8.16, bead `agents-xhli.24`). |
| **ISR nesting guard** | `CONFIG_XTENSA_ISR_NESTING_GUARD` (carried patch 0017, every ESP32 board): the interrupt entry treats a nesting count below 1 as "not nested" and switches stacks, the exit floors it at 0, both counted in RTC memory, and a C-side detector keeps the first ISR that leaves the count wrong. Exists because the first decoded class-B record showed `nested == −1` in idle: with the count wrong every later interrupt runs on a thread stack and the node dies silently. This stops the consequence and names the ISR. |
| **Vendor-blob guard** | The ESP32-S3 BLE controller is a binary written to FreeRTOS conventions; carried patch 0019 guards its OS-interface seam in the `hal_espressif` adapter: `_task_yield_from_isr` (was `k_yield()` — a context switch from inside an interrupt on Zephyr) is a counted no-op in ISR context, blocking waits from ISR become non-blocking and are counted, the "task woken" flag the blob reads is defined, and every controller ISR runs behind a guard that restores the kernel nesting count if the handler changed it. Counters in `struct esp_bt_osi_stats`; the watchdog heartbeat logs a `WRN` the first time any anomaly counter moves; `meshtastic resets` prints the retained kernel-side ones. |
| **Backtrace on fatal** | `CONFIG_XTENSA_ENABLE_BACKTRACE` on the ESP32-S3 (carried patch 0016 — upstream only offered it to the original ESP32); `FAULT_DUMP=2` + `EXTRA_EXCEPTION_INFO` on the nRF52840. |
| **Kernel self-checks** | `ASSERT=y` + `ASSERT_VERBOSE` everywhere; `SPIN_VALIDATE` (double-lock / wrong-unlock) on both SoCs; `STACK_CANARIES_STRONG` (a canary in every function with a local array or an address-taken local, not only the large frames); `DEBUG_THREAD_INFO` (+ the coredump threads-metadata section on ARM) for thread-aware post-mortems; HCI error codes as strings in the BT log. Board-level defaults in each `Kconfig.defconfig` (ESP32 boards) and the XIAO shield's; the canary strength is a Kconfig `choice`, so it lives in `prj.conf`. |
