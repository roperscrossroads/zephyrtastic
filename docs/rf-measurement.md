...
| `AGC reset` | `ok` / `fail` / `skipped` / `deferred` / `radio-busy` / `patch-fail` | the 60 s warm-sleep cycle that keeps the SX126x AGC from latching. `skipped` = a transmit was in flight at the chip; `deferred` = a packet was being demodulated, so the reset stood back 1 s rather than lose it; `radio-busy` = the radio was held for over a second (a transmit stuck waiting for its TX-done), so the reset stood back rather than block the system workqueue — on a XIAO that queue feeds the boot-guard watchdog. `fail`/`patch-fail` are the bad ones. |
